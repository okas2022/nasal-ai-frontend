...
$("analyzeBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const rows = Object.keys(data.ratios)
      .map(k => `<tr><td>${k}</td><td>${mk(data.ratios[k])}</td><td>${mk(data.normal_ranges[k])}</td><td>${mk(data.deviation[k])}</td></tr>`)
      .join("");
    $("resultTable").innerHTML = `<tr><th>항목</th><th>측정</th><th>정상 기준</th><th>편차</th></tr>${rows}`;
    ...
    chart = new Chart($("riskChart").getContext("2d"), {
      ...
      data: {
        labels,
        datasets: [
          { label: "측정값(%)", data: vals, backgroundColor: "rgba(0,102,204,0.7)" },
          { label: "정상기준(%)", data: norms, backgroundColor: "rgba(102,204,255,0.3)" }
        ]
      },
